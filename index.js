...
import { agentLoop, TOOL_DEFINITIONS, hasToolCall } from "./tools/agent.js";
import {
...
- No explanation, just JSON`;
  ...
  enqueue(async () => {
    ...
    try {
      ...
      // Use streaming if not in an agentic tool-call scenario
      let reply;
      if (hasToolCall("")) {
        // agentic path — no streaming (tool calls need full response)
        reply = await agentLoop((msgs) => chatWithRetry(msgs), augmented, userId);
        saveMessage(chatId, "assistant", reply);
        await bot.sendMessage(chatId, reply, { parse_mode: "Markdown" });
      } else {
        // streaming path — words appear as they generate
